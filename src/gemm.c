    ...
    for (i = 0; i < M; ++i) {
        for (k = 0; k < K; ++k) {
            ...
            for (j = 0; j < N; ++j) {
                C[i*ldc + j] += A_PART*B[k*ldb + j];
                // if (j == 0 ) {
                //     printf("Test result C[0]:%f,A_PART*B:%f,A_PART:%f,B[%d]:%f\n", C[0], A_PART*B[k*ldb + j], A_PART, k*ldb + j, B[k*ldb + j]);
                // }
                // if(j%416 == 0 && pp < 3 && k == 0){
                //     pp++;

                //     printf("A_PART: %f, B[%d]: %f, A*B: %f, C[%f], result[%d]: %f \n", A_PART, j, B[k*ldb + j], A_PART*B[k*ldb + j], (i*ldc + j), C[i*ldc + j], C[i*ldc + j] + A_PART*B[k*ldb + j]);
                //     printf("A_PART: %f, B[%d]: %f, A*B: %f, C[%f], result[%d]: %f \n", A_PART, j+1, B[k*ldb + j+1], A_PART*B[k*ldb + j+1], (i*ldc + j+1), C[i*ldc + j+1], C[i*ldc + j+1] + A_PART*B[k*ldb + j+1]);
                //     printf("A_PART: %f, B[%d]: %f, A*B: %f, C[%f], result[%d]: %f \n\n\n", A_PART, j+2, B[k*ldb + j+2], A_PART*B[k*ldb + j+2], (i*ldc + j+2), C[i*ldc + j], C[i*ldc + j+2] + A_PART*B[k*ldb + j+2]);
                // }
            }
            // printf("\n\n\n");
        }
...
void activate_array_cpu_custom(float *x, const int n, const ACTIVATION a)
{
    printf("[activate_array_cpu_custom] ");
    int i;
    if (a == LINEAR)
    {
        printf("activation: LINEAR and do NOTHING\n");
    }
    else if (a == LEAKY)
    {
        printf("activation: LEAKY and if value < 0 then val * .1\n");
        for (i = 0; i < n; ++i) {
            x[i] = (x[i]>0) ? x[i] : .1*x[i];
        }
    }
    else {
        printf("activation not linear or leaky  \n");
        for (i = 0; i < n; ++i) {
            x[i] = activate(x[i], a);
        }
    }
    ...
    int pad, int stride, int batch)
{
    ...
    printf("w_offset: %d, h_offset: %d, c: %d, out_h: %d, out_w: %d, size: %d, \n", w_offset, h_offset, c, out_h, out_w, size);
        ...
        float *C, int ldc)
{
    ...
    if (is_fma_avx2() && !TA && !TB) {
        ...
        int t;
        #pragma omp parallel for
        // printf("[gemm_nn] M: %d, K: %d, N: %d, lda: %d, ldb: %d, ldc: %d\n", M, K, N, lda, ldb, ldc); 
        for (t = 0; t < M; ++t) { 
            if (!TA && !TB) // huda: masuk kesini ya
