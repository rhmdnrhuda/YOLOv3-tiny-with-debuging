...
void forward_network(network net, network_state state)
{
    ...
    int i;
    printf("-------------------------------------------------------------------\nStart forward network %d times network \n", net.n);
    for(i = 0; i < net.n; ++i){
        state.index = i;
        layer l = net.layers[i];
        if (l.type == CONVOLUTIONAL){
            // printf("[iteration: %d] Convolutional Layer number of biases %d, %f\n", i, l.nbiases, l.biases[0]);
        } else if (l.type == MAXPOOL){
            // printf("[iteration: %d] MAXPOOL Layer\n", i);
        } else if (l.type == YOLO){
            printf("[iteration: %d] YOLO Layer\n", i);
        }
        printf("size input index 0: %f, channels: %d, outputs: %d\n", state.input[0], l.c, l.outputs);
        if(l.delta && state.train){
            //abaikan ini yaa gaada yang masuk waktu detector test 0001.jpg
        ...
        printf("size output index 0: %f , outputs: %d\n", l.output[0], l.outputs);
        printf("-------------------------------\n\n");
...
float *get_network_output(network net)
{
    ...
    int i;
    for(i = net.n-1; i > 0; --i) {
        printf("[network.c] [*get_network_output] %u :  %d, ", net.layers[i].type, i);
...
float *network_predict(network net, float *input)
{
    ...
    state.train = 0;
    state.delta = 0;
    // printf("[network.c] [*network_predict] [*state.input] before call forward net:  %f \n", *state.input);

    forward_network(net, state);

    // printf("[network.c] [*network_predict] [*state.input] after call forward net :  %f \n", *state.input);
    

    // float *out = get_network_output(net);

    // printf("[network.c] [*network_predict] [*out] %f\n", *out);
    // return out;
}
